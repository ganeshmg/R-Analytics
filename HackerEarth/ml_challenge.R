...
#Hacker earth ML Challenge
#18 March 2017

setwd("C:/Users/gajendir/OneDrive - Hewlett Packard Enterprise/Ganesh/Learning/Technical/R/Hackerearth/MLChallenge_Data_Mar2017")

dir()
train = read.csv("train_indessa.csv",stringsAsFactors = FALSE)
test = read.csv("test_indessa.csv",stringsAsFactors = FALSE)
str(train)
summary(train)
names(train)
names(test)
dim(train)
dim(test)
unique(train$loan_status)
class(train$loan_status)
md.pattern(train) -> data_pattern
rm(data_pattern)



#Example Code - GBM, H2o
